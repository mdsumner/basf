...
#' the plot unusable for subsequent additional drawing. The key feature is that
#' we don't have to change our behaviour and good habits depending on the
#' format in use.
...
plot.sf <- function(x, ...) {
  ## avoid obstructive errors for non-sf created objects
  attr(x[[attr(x, "sf_column")]], "crs") <-
    structure(list(input = NA_character_, wkt = NA_character_), class = "crs")
